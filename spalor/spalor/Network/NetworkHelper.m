...
-(NSMutableURLRequest *)formRequestwithemail:(NSString *)emailId andURLString:(NSString *)urlString andToken:(NSString *)token{
    ...
    DLog(@"AESCrypt %@ %@ token %@",encryptedAuth,[AESCrypt decrypt:encryptedAuth password:AESKey],token);
...
- (void)getArrayFromPutURL:(NSString *)url parmeters:(NSDictionary *)dicParams completionHandler:(void (^)(id response, NSString *url, NSError *error))block {
    ...
    NSMutableString *urlString = [NSMutableString stringWithFormat:@"%@/%@?_request_id=%@&_api_version_id=%@",INDULGE_URL,url,token,INDULGE_API_VERSION];
    ...
    if(user.mail!=nil){
        ...
        request = [self formRequestwithemail:user.mail andURLString:urlString andToken:token];
...
//    NSString *authToken = [sessionDict objectForKey:@"auth_token"];
//    
//    NSMutableString *urlString = [NSMutableString stringWithFormat:@"%@/%@?token=%@&_api_version_id=%@",INDULGE_URL,url,APIRequestID(),INDULGE_API_VERSION];
//    
//    
...
- (void)getArrayFromPostURL:(NSString *)url parmeters:(NSDictionary *)dicParams completionHandler:(void (^)(id response, NSString *url, NSError *error))block {
    ...
    NSString *stringData = [self prepareBody:dicParams];
    NSData *data = [stringData dataUsingEncoding:NSUTF8StringEncoding];
    NSMutableString *urlString = [NSMutableString stringWithFormat:@"%@/%@?token=%@&",INDULGE_URL,url,token];
    ...
    //Only if its not the save query
    if(user.mail!=nil){
